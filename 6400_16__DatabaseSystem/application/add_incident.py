...
from application.database import commit_db


def new_incident_sql(incident_id, username, desp, lat, lng, inc_date):
    t = Template("""
        INSERT INTO `emergency_response_system`.`incident`
        VALUES ('$incident_id', '$username', '$description','$latitude', '$longitude',
        '$incident_date')
    """)
    return t.safe_substitute({'incident_id':incident_id, 'username': username,'description': desp,
                              'latitude': lat, 'longitude': lng, 'incident_date': inc_date})
